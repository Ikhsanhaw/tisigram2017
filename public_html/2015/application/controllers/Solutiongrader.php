 ...
 *
 * @package controllers
 * @author Muhammad Saiful Islam <muhammad@saiful.web.id>
 */
class Solutiongrader extends CI_Controller 
{
	/**
	...
	public function run()
	{
		...
		$this->logger->log('grader', 'Solution grader started.');
	...
	{
		$q = $this->db->query('UPDATE grader SET last_activity=NOW() WHERE hostname="' . php_uname('n') . ' [sol]"');
	}
	...
	{
		$q = $this->db->query('SELECT COUNT(id) AS cnt FROM solution_submission WHERE verdict<=0 LIMIT 1');
		$res = $q->row_array();
		return $res['cnt'] > 0;
	}
	...
	{
		$q = $this->db->query('SELECT * FROM solution_submission WHERE verdict<=0 ORDER BY id ASC LIMIT 1');
		if ($q->num_rows() == 0)
			return FALSE;
		return $q->row_array();
	}
	...
	private function grade_submission($submission)
	{
		...
		$solution_detail = $this->get_source($submission['solution_id']);
		$language = $this->get_language($solution_detail['language_id']);
		$problem = $this->get_problem($solution_detail['problem_id']);

		$msg  = 'Found new submission!' . "\n";
		$msg .= '                      Submission     : ' . $submission['solution_id'] . "\n";
		$msg .= '                      Problem        : ' . $problem['name'] . "\n";
		...
		{
			$this->db->query('UPDATE solution_submission SET start_judge_time="' . date('Y-m-d H:i:s') . '" WHERE id=' . $submission['id']); 
			...
			$this->db->query('UPDATE solution_submission SET end_judge_time="' . date('Y-m-d H:i:s') . '" WHERE id=' . $submission['id']); 
		}
	...
	{
		$solution_detail = $this->get_source($submission['solution_id']);
		$submission_path = 'secret-solution/' . $solution_detail['problem_id'] . '/' . $submission['solution_id'] ;
		...
		if (empty($filter_result))
		{
			...
			foreach ($output as $v) {
				$compile_result .= str_replace(array($submission_path . '/source/',
					'/usr/bin/ld: warning: link.res contains output sections; did you forget -T?'), array('', ''), $v) . "\n";
			}
	...
	{
		$this->db->query('UPDATE solution_submission SET verdict=' . $verdict . ' WHERE id=' . $submission['id']);
	...
	{
		$solution_detail = $this->get_source($submission['solution_id']);
		$testcases = $this->get_testcases($solution_detail['problem_id']);
		$checker = $this->get_checker($solution_detail['problem_id']); // empty array if no checker
		...
		{
			$tc_path = $this->setting->get('testcase_path') . '/' . $solution_detail['problem_id'];
			...
			else
			{
				...
				else
				{
					...
					$diff_cmd = 'diff --strip-trailing-cr -q ' . $tc_path . '/' . $v['output'] . ' ' . $out_path . '/' . $v['output'] . ' > ' . $out_path . '/diff';
			...
			$this->db->query('INSERT INTO solution_judging(solution_submission_id, testcase_id, time, memory, verdict) VALUES(' . $submission['id'] . ', ' . $v['id'] . ', ' . $run_result_time . ', ' . $run_result_memory . ', ' . $verdict . ')');
	...
	private function get_source($solution_id)
	{
		$q = $this->db->query('SELECT * FROM solution WHERE id=' . $solution_id);
		return $q->row_array();
	}
